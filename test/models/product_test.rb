...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'product is not valid without a unique title' do
    ...
    assert product.invalid?
    # Active Record built in error message
    assert_equal [I18n.translate('errors.messages.taken')], product.errors[:title]
  end

  test 'product title should have at least 10 characters' do
    bad_product = new_product('fred.gif')
    bad_product.title = '123456789'
    assert bad_product.invalid?

    ok_product = new_product('fred.gif')
    ok_product.title = '1234567890'
